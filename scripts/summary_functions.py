...
def weighted_average(
    df_in, col, weights, grouper=None
):  # Computes the weighted average. Grouping by another column returns a series instead of a number
    if grouper == None:
...
def get_differences(
    ...
    df["% Difference"] = (
        (df["Difference"] / df[colname2] * 100).astype("float").round(2)
    )
...
def get_counts(
    counts_df, input_time
):  # Function to get counts for a SoundCast time period
    count = 0
    if input_time[1] == " ":
...
def to_percent(
    number,
):  # Converts a number to a string that is the number followed by a percent sing
...
):  # For a mode share, converts the columns to strings of numbers with percent signs

...
def calculate_auto_cost(trip_purpose, skim_dict, parking_cost_array, parameters_df):
    ...
    occupancy_factor = {"sov": 1, "hov2": 2, "hov3": 3.5}

    for mode in ["sov", "hov2", "hov3"]:
        auto_cost_dict[mode] = (
            skim_dict["distance"][mode] * auto_op_cost
            + (skim_dict["cost"][mode]) / occupancy_factor[mode]
        )
...
def calculate_mode_utilties(
    ...
    asc_dict = {
        "sov": 0,  # SOV is the reference mode so ASC=0
        "hov2": "asccs2",
        "hov3": "asccs3",
        "transit": "ascctw",
        "walk": "asccwk",
    ...
    for mode in ["sov", "hov2", "hov3", "walk", "bike"]:
        ...
        if mode not in ["walk", "bike"]:
            util = np.exp(
                asc
                + get_param(params_df, "autivt") * skim_dict["time"][mode]
    ...
    for submode1 in submode_dict.values():
        for submode2 in submode_dict.values():
            if submode2 != submode1:
                utility_matrices[submode2][
                    utility_matrices[submode1] >= utility_matrices[submode2]
                ] = 0
                utility_matrices[submode1][
                    utility_matrices[submode2] == utility_matrices[submode1]
                ] = 0
...
def main():
    ...
    total_trips_by_mode["sov"] = (
        airport_trips_by_mode["sov"] + ext_trip_table_dict["sov"]
    )
    total_trips_by_mode["hov2"] = (
        airport_trips_by_mode["hov2"] + ext_trip_table_dict["hov2"]
    )
    total_trips_by_mode["hov3"] = (
        airport_trips_by_mode["hov3"] + ext_trip_table_dict["hov3"]
    )

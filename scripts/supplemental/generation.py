...
def balance_trips(trip_table, bal_to_attractions, include_ext):
    for key, value in trip_purp_col.iteritems():
        ...
        if key not in bal_to_attractions:
            ...
            trip_table[value].loc[0:dictZoneLookup[HIGH_TAZ]] *= bal_factor
        ...
        # Balance productions to attractions for college trips
            ...
            trip_table[key].loc[0:dictZoneLookup[HIGH_TAZ]] *= bal_factor
...
def main():
    ...
    for purpose in xrange(purp_cols[0], purp_cols[1] + 1):
        ...
        for zone in xrange(1,HIGH_TAZ + 1):
            ...
            hhs1 = pd.DataFrame(hhs.iloc[dictZoneLookup[zone]])
            #nonhhs1 = pd.DataFrame(nonhhs.iloc[zone-1])
            nonhhs1 = pd.DataFrame(nonhhs.iloc[dictZoneLookup[zone]])
            #gq1 = pd.DataFrame(gq.iloc[zone-1])
            gq1 = pd.DataFrame(gq.iloc[dictZoneLookup[zone]])
            ...
            trips_by_purpose[str(purpose)].loc[dictZoneLookup[zone]] = dot1
            #nonhh_trips_by_purp[str(purpose)].loc[zone-1] = dot2
            nonhh_trips_by_purp[str(purpose)].loc[dictZoneLookup[zone]] = dot2
            #gq_trips[str(purpose)].loc[zone-1] = dot3
            gq_trips[str(purpose)].loc[dictZoneLookup[zone]] = dot3

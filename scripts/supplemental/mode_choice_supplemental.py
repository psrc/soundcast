...
def calculate_auto_cost(
    trip_purpose, auto_skim_dict, parking_cost_array, parameters_df
):
    ...
    auto_op_cost = parameters_df[
        (parameters_df["purpose"] == trip_purpose)
        & (parameters_df["variable"] == "autoop")
    ]["value"].values[0]

    # SOV
    auto_cost_matrices["dabct"] = (
        auto_skim_dict["dabds"] * auto_op_cost + auto_skim_dict["dabcs"]
    )

    # HOV2; divide 2 to account for occupancy
    auto_cost_matrices["s2bct"] = (
        auto_skim_dict["s2bds"] * auto_op_cost + auto_skim_dict["s2bcs"]
    ) / 2

    # HOV3+; assuming average occupancy of 3.5
    auto_cost_matrices["s3bct"] = (
        auto_skim_dict["s3bds"] * auto_op_cost + auto_skim_dict["s3bcs"]
    ) / 3.5
...
def calculate_mode_shares(trip_purpose, mode_utilities_dict):
    ...
    output_mode_share[output_mode_share_name[trip_purpose][0]] = (
    ...
    # Auto, shda
    output_mode_share[output_mode_share_name[trip_purpose][1]] = (
        mode_utilities_dict["euda"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][1]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][1]])
    ] = 0
    # 2 passengers auto, shs2
    output_mode_share[output_mode_share_name[trip_purpose][2]] = (
        mode_utilities_dict["eus2"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][2]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][2]])
    ] = 0
    # 3 passenger auto, shs3
    output_mode_share[output_mode_share_name[trip_purpose][3]] = (
        mode_utilities_dict["eus3"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][3]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][3]])
    ] = 0

    # Transit to walk, shtw
    output_mode_share[output_mode_share_name[trip_purpose][4]] = (
        mode_utilities_dict["eutw"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][4]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][4]])
    ] = 0

    # Light Rail to walk, shtw
    output_mode_share[output_mode_share_name[trip_purpose][5]] = (
        mode_utilities_dict["eurw"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][5]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][5]])
    ] = 0

    # Bike, shbk
    output_mode_share[output_mode_share_name[trip_purpose][7]] = (
        mode_utilities_dict["eubk"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][7]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][7]])
    ] = 0
    # Walk, shwk
    output_mode_share[output_mode_share_name[trip_purpose][8]] = (
        mode_utilities_dict["euwk"] / output_mode_share["eusm"]
    )
    output_mode_share[output_mode_share_name[trip_purpose][8]][
        np.isnan(output_mode_share[output_mode_share_name[trip_purpose][8]])
    ] = 0
...
def calculate_mode_utilties(
    ...
    utility_matrices["euda"] = np.exp(
        get_param(params_df, "autivt") * auto_skim_dict["dabtm"]
...
def main():
    ...
    for trip_purpose in trip_purpose_list:
        ...
        parking_costs = get_destination_parking_costs(
            parcels_file_name, zones, zone_lookup_dict
        )
        auto_cost_dict = calculate_auto_cost(
            trip_purpose, auto_skim_dict, parking_costs, parameters_df
        )

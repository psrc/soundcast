...
# os.chdir(r'L:\vision2050\soundcast\integrated\final_runs\rug\rug_run_5.run_2018_10_25_09_07\2050\scripts\summarize\notebooks')
# os.chdir(r'L:\vision2050\soundcast\integrated\final_runs\stc\stc_run_6.run_2018_10_23_11_15\2050\scripts\summarize\notebooks')
# os.chdir(r'L:\vision2050\soundcast\integrated\final_runs\tod\tod_run_8.run_2018_10_29_15_01\2050\scripts\summarize\notebooks')
os.chdir(r'L:\vision2050\soundcast\integrated\final_runs\base_year\2014\scripts\summarize\notebooks')

relative_path = '../../../outputs'
output_vision_file = os.path.join(relative_path, 'Vision2050_longlist_updated.csv')
...
def mode_geo_tot(trip, geog_level, output_dict, name):
    mode_geog = trip[[geog_level, 'mode', 'hhno']].groupby([geog_level, 'mode']).count()['hhno'].reset_index()
    # mode_geog = mode_df_geog.groupby([geog_level ,'mode']).sum()['hhno']/mode_df_geog.groupby(geog_level).sum()['hhno']
    # mode_geog=mode_geog.reset_index()
    # mode_df['Mode Totals'] = mode_df['hhno']
    mode_geog.columns = [geog_level, 'mode', 'Mode Totals']
    mode_geog.replace({'mode':mode_dict}, inplace=True)

    for index, row in mode_geog.iterrows():
        output_dict[(name, row[geog_level], row['mode'])]=row['Mode Totals']
...
def mode_totals(trip, person, output_dict):
	# Split into HBW/Non-HBW
    trip['Trip Type'] = 'Not Home-Based Work'
    trip.ix[(((trip['opurp']==0) & (trip['dpurp']==1)) | ((trip['opurp']==1) & (trip['dpurp']==0))),'Trip Type']= 'Home-Based Work'
    hbw_trips = trip.loc[trip['Trip Type']=='Home-Based Work']
    ...
    for index, row in mode_df.iterrows():
        output_dict[('All Trip Totals', 'Regional', row['mode'])]=row['Mode Totals']
    ...
    for index, row in mode_df.iterrows():
        output_dict[('Commute Trip Totals', 'Regional', row['mode'])]=row['Mode Totals']
    ...
    for index, row in mode_df.iterrows():
        output_dict[('Non-Commute Trip Totals', 'Regional', row['mode'])]=row['Mode Totals']

    # Mode share by geography
    for geog in geog_list:
        mode_geo_tot(trip, geog+'_tripdest', output_dict, 'All Trip Totals')
        mode_geo_tot(hbw_trips, geog+'_tripdest', output_dict, 'Commute Trip Totals')
        mode_geo_tot(nhbw_trips, geog+'_tripdest', output_dict, 'Non-Commute Trip Totals')
...
if __name__ == "__main__":
    ...
    output_dict = mode_totals(trip, person, output_dict)

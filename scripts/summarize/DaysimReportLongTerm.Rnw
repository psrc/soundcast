...
useful_group <- c("Household_1", "Household_2", "Person_1", "Person_2")
...
workers_1_avg_dist_ft <- weighted.mean(workers_1_ft$pwaudist, workers_1_ft$psexpfac)
workers_2_avg_dist_ft <- weighted.mean(workers_2_ft$pwaudist, workers_2_ft$psexpfac)

workers_1_avg_dist_pt <- weighted.mean(workers_1_pt$pwaudist, workers_1_pt$psexpfac)
workers_2_avg_dist_pt <- weighted.mean(workers_2_pt$pwaudist, workers_2_pt$psexpfac)

workers_1_avg_dist_f <- weighted.mean(workers_1_female$pwaudist, workers_1_female$psexpfac)
workers_2_avg_dist_f <- weighted.mean(workers_2_female$pwaudist, workers_2_female$psexpfac)

workers_1_avg_dist_m <- weighted.mean(workers_1_male$pwaudist, workers_1_male$psexpfac)
workers_2_avg_dist_m <- weighted.mean(workers_2_male$pwaudist, workers_2_male$psexpfac)

workers_1_avg_dist_age <-weighted.mean(workers_1_ageund30$pwaudist, workers_1_ageund30$psexpfac)
...
wrkrslessonemi_1 <- subset(workers_1, workers_1$pwaudist<=1)
workers_1_less_one_mi <- 100*sum(wrkrslessonemi_1$psexpfac)/sum(workers_1$psexpfac)
...
wrkrsgrtwentymi_1 <- subset(workers_1, workers_1$pwaudist>20)
workers_1_gr_twenty_mi <- 100*sum(wrkrsgrtwentymi_1$psexpfac)/sum(workers_1$psexpfac)
...
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=
wkr_1_hzone_sum <- aggregate(wkr_1_hzone$hhexpfac, by = list(wkr_1_hzone$County, wkr_1_hzone$pwtaz), sum)
...
wrk1_homecounty <- aggregate(wkr_1_hzone$hhexpfac, by = list(wkr_1_hzone$County), sum)
...
Work_1_o<-  sqldf("SELECT pwtaz,sum(psexpfac) as NumWorkers, 
...
students_1$share <- students_1$psexpfac/sum(students_1$psexpfac)
...
students_1_avg_dist <- weighted.mean(students_1$psaudist, students_1$psexpfac)
students_2_avg_dist <- weighted.mean(students_2$psaudist, students_2$psexpfac)

students_1_dist_und5 <- weighted.mean(students_1_und5$psaudist, students_1_und5$psexpfac)
students_2_dist_und5<- weighted.mean(students_2_und5$psaudist, students_2_und5$psexpfac)

students_1_dist_512 <-weighted.mean(students_1_512$psaudist, students_1_512$psexpfac)
students_2_dist_512<- weighted.mean(students_2_512$psaudist, students_2_512$psexpfac)

students_1_dist_1318 <-weighted.mean(students_1_1318$psaudist, students_1_1318$psexpfac)
students_2_dist_1318<- weighted.mean(students_2_1318$psaudist, students_2_1318$psexpfac)

students_1_dist_19P <- weighted.mean(students_1_19P$psaudist, students_1_19P$psexpfac)
...
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=
Total_Transit_Passes1<- sum(merge_per_hh_1$ptpass*merge_per_hh_1$hhexpfac)
...
auto_own1 <- aggregate(Household_1$hhexpfac, by = list(Household_1$hhvehs), sum)
...
hhs_county_1 <- aggregate(hhs_zone_1$hhexpfac, by = list(hhs_zone_1$County), sum)
colnames(hhs_county_1) <- c("County", "NumHHs")
#get autoOwnership
AutoOwnership_1 <- aggregate(hhs_zone_1$hhexpfac, by = list(hhs_zone_1$County, hhs_zone_1$hhveh), sum)
colnames(AutoOwnership_1) <- c("County", "Auto","NumHHs")
HH_Auto_County_1 <- matrix(AutoOwnership_1$ NumHHs, nrow = 4, ncol = 5)

HH_Auto_Total_1 <- aggregate(hhs_zone_1$hhexpfac, by = list(hhs_zone_1$hhveh), sum)
...
income_sep <- c(0, 20000, 40000, 60000, 75000, max(Household_1$ hhincome) + 1)
auto_val <- c(0, 1, 2, 3, 4)
income_group <- c("Less than $20000", "$20000-$39999", "$40000-$59999","$60000-$74999","$75000 or more") 
auto_ownership <- c("0 car", "1 car", "2 car", "3 car", "4+ car") 


HH_income_auto_1 <- matrix(rep(0,25), ncol = 5)

for (i in 1:5){
  index1 <- (Household_1$ hhincome >= income_sep[i])
  index2 <- (Household_1$ hhincome[index1] < income_sep[i + 1])
  for (j in 1:5){
    HH_income_auto_1[i,j] <- round(sum(Household_1$ hhveh[index1][index2] == auto_val[j])/length(Household_1$ hhveh[index1][index2]) * 100, 1)
  }
}

hhs_2 <- c(1437220,103900,467260,543625,219750,102665,33895,8950,17035,5720,1595,600,43345,17450,19290,4850,1165,575,25840,7940,12925,3785,770,415,25275,7060,13355,3435,950,470,27520,6340,15260,4400,985,540,24440,4460,13585,4870,1115,410,32065,5225,18055,6560,1540,690,26355,3930,14135,6525,1295,470,31905,3620,17840,8180,1480,795,26540,2915,14010,7145,1885,590,35620,3730,19375,9335,2400,780,28890,2365,15070,8675,1985,800,31265,2370,15740,9400,2845,910,30360,2085,14680,10005,2715,860,34195,2070,16195,11870,3000,1060,31690,1985,14610,11045,3080,970,31020,1590,13560,10870,3640,1350,28540,1250,12280,10555,3115,1340,61380,2825,24855,23555,7415,2720,54005,2220,20400,20845,7485,3060,157265,4275,47750,69405,25355,10475,207875,4185,46270,96060,42020,19330,149820,2120,23975,71005,35455,17255,89025,1150,9830,42510,23180,12365,169140,1785,17195,83030,43300,23825)
HH_income_auto_CTPP <- matrix(hhs_2, nrow = 6)
HH_income_auto_CTPP <- t(HH_income_auto_CTPP)
HH_income_auto_CTPP <- HH_income_auto_CTPP[-1, -1]

HH_income_auto_2 <- matrix(rep(0,25), nrow = 5)

for (j in 1:5){

  HH_income_auto_2[1,j] <- round(sum(HH_income_auto_CTPP[1:6,j])/sum(HH_income_auto_CTPP[1:6,]) * 100,1)               
  HH_income_auto_2[2,j] <- round(sum(HH_income_auto_CTPP[7:14,j]) /sum(HH_income_auto_CTPP[7:14,]) * 100,1)
  HH_income_auto_2[3,j] <- round(sum(HH_income_auto_CTPP[15:20,j]) /sum(HH_income_auto_CTPP[15:20,]) * 100,1) 
  HH_income_auto_2[4,j] <- round(sum(HH_income_auto_CTPP[21:21,j]) / sum(HH_income_auto_CTPP[21:21,]) * 100,1)
  HH_income_auto_2[5,j] <- round(sum(HH_income_auto_CTPP[22:25,j])/sum(HH_income_auto_CTPP[22:25,]) * 100,1)
}


HH_income_auto_1 <- data.frame(HH_income_auto_1)
rownames(HH_income_auto_1) <- income_group 
colnames(HH_income_auto_1) <- auto_ownership 

HH_income_auto_2 <- data.frame(HH_income_auto_2)
rownames(HH_income_auto_2) <- income_group
colnames(HH_income_auto_2) <- auto_ownership

print ("Model: Households by Income Group by Auto Ownership")
print (HH_income_auto_1)
print ("CTPP: Households by Income Group by Auto Ownership")
print (HH_income_auto_2)
...
workers_1$type <- names(paths[1])
workers_2$type <- names(paths[2])
workers <- rbind(workers_1,workers_2)

workers$type <- as.factor(workers$type)

workers_dist_graph <- ggplot(data= workers,
                           aes(x=pwaudist, weight=share,fill=type))
workers_dist_graph + geom_histogram(binwidth = 1, colour= "black",position= position_dodge(width = 1.0))+xlab("Auto Distance to Work (miles)") + ylab("Total Workers")+scale_x_continuous(limits = c(0, 50))+labs(title="Distance to Work Lengths")

students_1$type <- names(paths[1])
students_2$type <- names(paths[2])
students_2$id <- students_2$hhno
students <- rbind(students_1,students_2, use.names=TRUE)


#students_dist_graph <- ggplot(data= students,
#                           aes(x=psaudist, weight=share, fill=type))
#students_dist_graph + geom_histogram(binwidth = 1,colour= "black",position#= position_dodge(width = 1.0))+xlab("Auto Distance to School(miles)") + #ylab("Total Students")+scale_x_continuous(limits = c(0, 30))+labs(title#="Distance to School")

...
students <- rbind(students_1,students_2)
students_1$type <- names(paths[1])
students_2$type <- names(paths[2])
...
\section{Total used time and memory}
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=

print ("Total used time (s) is:")
print ((proc.time()-ptm)[3])
print ("Total used memory (MB) is:")#assume that we're using a 64-bit operation system
node_size <- 56L
usage <- gc()
print(sum(usage[, 1] * c(node_size, 8)) / (1024 ^ 2))
@
\end{document}
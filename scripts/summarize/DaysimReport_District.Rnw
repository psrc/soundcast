...
trip_ok_2<- subset(Trip_2, travdist>0 & travdist<2000)
 ...
 Trip_1_o<-  sqldf("SELECT dtaz,count(*) as NumTrips,
  ...
  Trip_1_od<-  sqldf("SELECT sum(NumTrips) as Trips,
                     OriginDistrict,
...
###Person_2 summaries
 ...
 Trip_2_o<-  sqldf("SELECT dtaz,sum(trexpfac)as NumTrips,
  ...
  Trip_2_od<-  sqldf("SELECT sum(NumTrips)as NumTrips,
                     OriginDistrict,
...
Trip_1_MTX<-  acast(Trip_1_od, formula=OriginDistrict ~ DestinationDistrict, 
                    value.var='Trips')

Trip_2_MTX<-  Trip_2_od


write.csv(Trip_1_MTX, paste(run_path,"/scripts/summarize/ModelTripsDistrict.csv", sep = ""))
write.csv(Trip_2_MTX, paste(run_path,"/scripts/summarize/SurveyTripsDistrict.csv", sep = ""))

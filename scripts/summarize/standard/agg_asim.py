...
def create_agg_outputs(path_dir_base, base_output_dir, survey=False):
    ...
    expr_df = pd.read_csv(os.path.join(os.getcwd(),r'inputs/model/summaries/activitysim/agg_expressions.csv'))
    # expr_df = expr_df.fillna('__remove__')    # Fill NA with string signifying data to be ignored
    geography_lookup = pd.read_csv(os.path.join(os.getcwd(),r'inputs/model/summaries/activitysim/geography_lookup.csv'))
    variables_df = pd.read_csv(os.path.join(os.getcwd(),r'inputs/model/summaries/activitysim/variables.csv'))
    global labels_df
    labels_df = pd.read_csv(os.path.join(os.getcwd(),'inputs/model/lookup/variable_labels.csv'))
    
    # FIXME: add geography at MAZ level
    ...
    # Expression log
    expr_log_path = r'outputs/agg/activitysim/expr_log.csv'
    if os.path.exists(expr_log_path):
        os.remove(expr_log_path)

    hh_full_col_list = pd.read_csv(os.path.join(path_dir_base,'final_households.csv'), nrows=0).columns.values
    person_full_col_list = pd.read_csv(os.path.join(path_dir_base,'final_persons.csv'), nrows=0).columns.values
    trip_full_col_list = pd.read_csv(os.path.join(path_dir_base,'final_trips.csv'), nrows=0).columns.values
    tour_full_col_list = pd.read_csv(os.path.join(path_dir_base,'final_tours.csv'), nrows=0).columns.values
    ...
    full_col_list = list(hh_full_col_list) + list(person_full_col_list) + list(trip_full_col_list) + list(tour_full_col_list) + var_list
    ...
    for index, row in df_agg.iterrows():
        ...
        fname = os.path.join(base_output_dir,survey_str,str(row['target']))
    ...
    for index, row in df_agg.iterrows():
        ...
        # households
        # Also account for any added user variables
        load_cols = [i for i in col_list if i in hh_full_col_list] + ['household_id']
        household = pd.read_csv(os.path.join(path_dir_base,'final_households.csv'), usecols=load_cols)

        # persons
        # Also account for any added user variables
        load_cols = [i for i in col_list if i in person_full_col_list] + ['person_id']
        person = pd.read_csv(os.path.join(path_dir_base,'final_persons.csv'), usecols=load_cols)

        # tours
        # Also account for any added user variables
        load_cols = list(np.unique([i for i in col_list if i in tour_full_col_list] + ['household_id','person_id']))
        # only load user variables that are related to this table
        user_var_cols = [i for i in col_list if i in variables_df['new_variable'].values]

        if len(user_var_cols) > 0:
            df_var = variables_df[variables_df['new_variable'].isin(col_list)]
            load_cols += df_var[~df_var['modified_variable'].isnull()].values.tolist()
        # also load any columns needed for geographic join
        # geog_cols = [i for i in col_list if i in geography_lookup['right_column_rename'].values]
        # if len(geog_cols) > 0:
        ...
        tour = pd.read_csv(os.path.join(path_dir_base,'final_tours.csv'), usecols=load_cols)
        ...
        tour = tour.merge(household, on=['household_id'])
        if len(person) > 0:
            tour = tour.merge(person, on=['person_id'])
...
def copy_dash_tables(dash_table_list):
    ...
    for fname in dash_table_list:
        shutil.copy(os.path.join(r'outputs/network',fname+'.csv'), r'outputs/activitysim/agg')

def main():

    output_dir_base = os.path.join(os.getcwd(),'outputs/activitysim/agg')
    create_dir(output_dir_base)

    input_dir = os.path.join(os.getcwd(),r'outputs/activitysim')
    create_agg_outputs(input_dir, output_dir_base, survey=False)

    # survey_input_dir = os.path.join(os.getcwd(),r'inputs/base_year/survey/activitysim')

...
def aggregate_travel_time(travel_time_df, taz_df, mode):
    travel_time_df = travel_time_df.merge(
        taz_df, how="left", left_on="to", right_on="TAZ_P"
    )
    travel_time_df = (
        travel_time_df.groupby("from")
        .aggregate({"EMPTOT_P": sum, "HH_P": sum})
        .reset_index()
    )
    travel_time_df = travel_time_df.rename(
        columns={"EMPTOT_P": "EMPTOT_P" + "_" + mode, "HH_P": "HH_P" + "_" + mode}
    )
...
taz_data = (
    parcel_df.groupby("TAZ_P").aggregate({"EMPTOT_P": sum, "HH_P": sum}).reset_index()
)

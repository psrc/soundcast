...
toursbypurpose1 <- aggregate(Tour_1$toexpfac, by = list(Tour_1$pdpurp), sum)
...
toursbypurpose1_Person <- data.frame(cbind(toursbypurpose1$pdpurp, round(toursbypurpose1$Tour/Person_1_total,3)))
colnames(toursbypurpose1_Person) <- c("pdpurp", "TourPerPerson")

toursbypurpose2_Person <- data.frame(cbind(toursbypurpose2$pdpurp, round(toursbypurpose2$Tour/Person_2_total,3)))

colnames(toursbypurpose2_Person) <- c("pdpurp", "TourPerPerson")

tours_by_purpose_pers <- merge(toursbypurpose1_Person, toursbypurpose2_Person, by = "pdpurp")

print(tours_by_purpose_pers)

PersonsDay1 <- merge(Person_1, PersonDay_1, by="id")
PersonsDay2 <- merge(Person_2, PersonDay_2, by=c("hhno", "pno"))

toursPersWork1<-aggregate(PersonsDay1$wktours, by=list(PersonsDay1$pptyp),FUN=mean, weights =PersonsDay1$psexpfac)
...
toursPersSchool1<-aggregate(PersonsDay1$sctours, by=list(PersonsDay1$pptyp), FUN=mean, weights =PersonsDay1$psexpfac)
...
toursPersEsc1<-aggregate(PersonsDay1$estours, by=list(PersonsDay1$pptyp), FUN=mean, weights =PersonsDay1$psexpfac)
...
toursPersShop1<-aggregate(PersonsDay1$shtours, by=list(PersonsDay1$pptyp), FUN=mean, weights =PersonsDay1$psexpfac)
...
toursPersMeal1<-aggregate(PersonsDay1$mltours, by=list(PersonsDay1$pptyp), FUN=mean, weights =PersonsDay1$psexpfac)
...
toursPersSo1<-aggregate(PersonsDay1$sotours, by=list(PersonsDay1$pptyp), FUN=mean, weights =PersonsDay1$psexpfac)
...
person_day_hh1 <- merge(PersonDay_1, Household_1, by = "hhno")
person_day_hh2 <- merge(PersonDay_2, Household_2, by = "hhno")

# if stops- Work
ave_work_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$wkstops),sum)
...
# school stops
ave_sc_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$scstops),sum)
...
ave_es_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$esstops),sum)
...
#shop
ave_sh_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$shstops),sum)
...
#meal
ave_ml_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$mlstops),sum)
...
#Personal Business
ave_pb_stops1 <- aggregate(100*person_day_hh1$hhexpfac/(Person_1_total),by=list(person_day_hh1$pbstops),sum)
...
#Social
ave_so_stops1 <- aggregate(100*person_day_hh1$hhexpfac/Person_1_total,by=list(person_day_hh1$sostops),sum)
...
TripsPerTour_1 <- aggregate(Tour_1$Trips, by =list(Tour_1$pdpurp), FUN=mean, weights=Tour_1$toexpfac)
...
Trip_1_AveDist <- weighted.mean(Trip_1$travdist[Trip_1$travdist>0 & Trip_1$travdist<200], Trip_1$trexpfac[Trip_1$travdist>0 & Trip_1$travdist<200])
...
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=
trips_by_purpose1 <- aggregate(Trip_1$trexpfac, by = list(Trip_1$dpurp), sum)

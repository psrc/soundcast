...
\begin{abstract}
This document compares the SoundCast M2 run to the household survey data.
...
paths <-c("C:/DaysimOutputsForTesting/daysim_outputs_M2.h5",
          "C:/DaysimOutputsForTesting/survey_new.h5")
dist_sum <- c("C:/DaysimOutputsForTesting/district_summary")

# Give a descriptive name of the two datasets that are being compared
names(paths)<- c("SoundCast_M2", "Survey")
type <- names(paths)
...
This table compares the total households by district between the two datasets.Note that the survey population has double the households in Shoreline as the synthetic population.  However, the synthetic population looks correct: Shoreline had 20,752 hhs in 2010 according to the City of Shoreline data.
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=
...
if(sum(Household_1$hhexpfac)<1){
  hhs_district_1 <-sqldf("SELECT District, count(*) as H from Household_1
...
hhs_district_2 <- sqldf("SELECT District, count(*) as H from Household_2
                        INNER JOIN zone_district
...
diff_hhs <- sqldf("SELECT hhs_district_1.District, hhs_district_1.H,
                  ...
                  ROUND(100*(hhs_district_1.H-hhs_district_2.H)/hhs_district_2.H,2)
...
PerDiffHHSize <- 100*(Average_HHSize_1 - Average_HHSize_2)/Average_HHSize_1
...
<<summary, cache=TRUE, eval=TRUE, echo=FALSE, include=TRUE, results = verbatim>>=
if(sum(Tour_1$toexpfac)<1)
{
  toursbypurpose1 <- sqldf("SELECT PDPURP, count(*) as Tour
                         from Tour_1 group by PDPURP order by PDPURP")

  toursbymode1 <- sqldf("SELECT tmodetp,
                      count(*) as Tour from Tour_1
                      group by tmodetp order by tmodetp")
}else{toursbypurpose1 <- sqldf("SELECT PDPURP, sum(toexpfac) as Tour
                         from Tour_1 group by PDPURP order by PDPURP")
...
if(sum(Tour_2$toexpfac)<1)
{ toursbymode2 <- sqldf("SELECT tmodetp,
                     count(*) as Tour from Tour_2
                      group by tmodetp order by tmodetp")
  toursbypurpose2 <- sqldf("SELECT PDPURP, count(*)as Tour
                         from Tour_2 group by PDPURP order by PDPURP")

}else{toursbymode2 <- sqldf("SELECT tmodetp,
...
tours_by_purpose<-sqldf("SELECT toursbypurpose1.PDPURP,
                        ...
                        toursbypurpose1.Tour-toursbypurpose2.Tour,
                       100* ((toursbypurpose1.Tour-toursbypurpose2.Tour)/
...
tours_by_mode<-sqldf("SELECT toursbymode1.tmodetp, toursbymode1.Tour,
                     ...
                     100*(toursbymode1.Tour-toursbymode2.Tour)/toursbymode1.Tour
...
##Trip_1 summaries
if(sum(Trip_1$trexpfac)<1){Trip_1_mode_time  <- ddply(Trip_1, .(mode), summarise,
  ...
  Trip_1_dest_d <- sqldf("SELECT count(*) as Trip1, District FROM Trip_1
                       INNER JOIN zone_district
                       on Trip_1.dtaz= zone_district.TAZ
                       Group by District")


  Trip_1_o<-  sqldf("SELECT dtaz,count(*) as NumTrips,
                  District as OriginDistrict FROM Trip_1
                  INNER JOIN zone_district
                  on Trip_1.otaz= zone_district.TAZ
                  where deptm>=360 and deptm<540
...
{
  Trip_2_dest_d <- sqldf("SELECT count(*) As Trip2, District FROM Trip_2
                       INNER JOIN zone_district
  ...
  Trip_2_mode_time  <- ddply(Trip_2, .(mode), summarise,
                           total_trips  =  nrow(travtime),
                           mode_share = nrow(travtime)/Trip_2_total,
                           mean_time = mean(travtime),
                           max_time = max(travtime),
                           min_time = min(travtime),
                           mean_dist = mean(travdist),
  ...
  Trip_2_o <-  sqldf("SELECT dtaz, count(*) as NumTrips,
  ...
  Trip_2_od <-  sqldf("SELECT sum(NumTrips) as Trips, OriginDistrict,
                    ...
                    Group by OriginDistrict,zone_district.District")
}else{Trip_2_dest_d <- sqldf("SELECT sum(trexpfac) As Trip2, District FROM Trip_2
                       INNER JOIN zone_district
                       on Trip_2.dtaz= zone_district.TAZ
                       Group by District")
...
Trip_1_2_dests <- sqldf("SELECT Trip_1_dest_d.District, Trip1, Trip2,
                        ...
                        ROUND(100*(Trip1-Trip2)/Trip1,2) As PerDiff
                        from Trip_1_dest_d
...
Trip_2_deptms <- data.frame(cbind(Trip_2$deptm,Trip_2$trexpfac/Trip_2$trexpfac))

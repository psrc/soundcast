...
\begin{abstract}
This document is comparing the results from two model runs. One is a run with the shortened roster list on the survey population. The other is a run with the full roster
list on the survey population
\end{abstract}
<<echo=false>>=
options(width=100)
@
...
paths <-c("C:/DaysimOutputsForTesting/daysim_outputs4.h5",
          "C:/DaysimOutputsForTesting/daysim_outputs8.h5")

# Give a descriptive name of the two datasets that are being compared
names(paths)<- c("ShortenedRoster", "FullRoster")
type <- names(paths)
...
if(sum(Trip_2$trexpfac)<1){
  ...
  Trip_2_dest_d <- sqldf("SELECT sum(trexpfac), District FROM Trip_2
                         INNER JOIN zone_district
                         on Trip_2.dtaz= zone_district.TAZ
                         Group by District")
  ...
  Trip_2_mode_purpose <- ddply(Trip_2 , .(mode,dpurp), summarise,
                               ...
                               mean_dist = mean(travdist),
                               type = paste(type[2]))
  Trip_2_6_9_mode_purpose <- ddply(Trip_2_6_9 , .(mode,dpurp), summarise,
                                   total_trips =  sum(trexpfac),
                                   mean_time = mean(travtime),
                                   mean_dist = mean(travdist),
                                   type = paste(type[2], "6 to 9"))

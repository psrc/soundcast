...
Tour_1$tour_id <- Tour_1$id

merge_trip_tour_1 <-  merge(Tour_1, Trip_1, by= "tour_id",allow.cartesian=TRUE)

trip_tour_mode_1 <- aggregate(merge_trip_tour_1$trexpfac, by= list(merge_trip_tour_1$tmodetp, merge_trip_tour_1$mode), length)

merge_trip_tour_2 <-merge(Tour_2, Trip_2, by= c("hhno", "pno","tour"),allow.cartesian=TRUE)

trip_tour_mode_2 <- aggregate(merge_trip_tour_2$trexpfac, by= list(merge_trip_tour_2$tmodetp, merge_trip_tour_2$mode), sum)

colnames(trip_tour_mode_1) <-c("Tour Mode", "Trip Mode", "Total Trips")
colnames(trip_tour_mode_2) <-c("Tour Mode", "Trip Mode", "Total Trips")

print("Trip Mode by Tour Mode MODEL")
print (trip_tour_mode_1)

print("Trip Mode by Tour Mode Survey")
print (trip_tour_mode_2)

sov_bike<-merge_trip_tour_1[merge_trip_tour_1$tmodetp=='sov' & merge_trip_tour_1$mode=='bike']

sov_bike_purpose <- aggregate(sov_bike, by=list(sov_bike$pdpurp), length)

sov_bike_pt <- aggregate(sov_bike, by=list(sov_bike$tpathtp), length)

sov_bike_o_d<-aggregate(sov_bike, by=list(sov_bike$otaz, sov_bike$dtaz), length)

write.csv(sov_bike,"C:\\sov_bike_new.csv")
...
Tour_1_mode_time  <- ddply(Tour_1, .(tmodetp), summarise,
                           mean_time = mean(tautotime),
                           mean_dist = mean(tautodist),
                           type = type[1])
...
Trip_1_mode_time  <- ddply(Trip_1, .(mode), summarise,
  ...
  Tour_2_mode_time  <- ddply(Tour_2, .(tmodetp), summarise,
                           mean_time = mean(tautotime),
                           mean_dist = mean(tautodist),
                           type = type[2])
...
print("647,127")
print("Assuming 1.3 Boardings per Trip- Implied Trips from Boardings(2011)")
transit<-Trip_1_mode_time$total_trips[Trip_1_mode_time$mode=='transit']
implied_boardings<-transit*1.3 
print(implied_boardings)

print(c("Tours by Mode and Travel Time", names(paths[1])))
print(Tour_1_mode_time, row.names=F)
print(c("Tours by Mode and Travel Time", names(paths[2])))
print(Tour_2_mode_time, row.names=F)
...
append_1_2_tour <- rbind(Tour_1_mode_time, Tour_2_mode_time)

tours_compare_distance <- ggplot(data=append_1_2_tour,aes(x=tmodetp, y= mean_dist, fill=tmodetp))
tours_compare_distance+ geom_bar(stat = "identity") +facet_wrap(~ type)+theme(text = element_text(size=10),axis.text.x = element_text(angle=90, vjust=1))+scale_fill_brewer(palette="Spectral")+ xlab("Mode")+ylab("Average Distance")

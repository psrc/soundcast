...
toursbymode1 <- aggregate(Tour_1$toexpfac, by = list(Tour_1$tmodetp), sum)
...
Tour_1_total_p <- aggregate(Tour_1$toexpfac, by = list(Tour_1$pdpurp), sum)
...
toursbymode1_p <- aggregate(Tour_1$toexpfac, by = list(Tour_1$tmodetp, Tour_1$pdpurp), sum)
...
trip_tour_mode_1 <- aggregate(merge_trip_tour_1$trexpfac, by= list(merge_trip_tour_1$tmodetp, merge_trip_tour_1$mode), sum)
...
Trip_1_time<- aggregate(Trip_1$trexpfac, by = list(Trip_1$deptm/60), sum)

...
for y in range (0, len(origin_destination_dict["Origin_Matrices"])):
    my_project.create_matrix('ORIGIN', origin_destination_dict['Origin_Matrices'][y]['Name'], 
                             origin_destination_dict['Origin_Matrices'][y]['Description'], 0, True, my_project.current_scenario)
...
for y in range (0, len(origin_destination_dict["Destination_Matrices"])):
    my_project.create_matrix('DESTINATION', origin_destination_dict['Destination_Matrices'][y]['Name'], 
                             origin_destination_dict['Destination_Matrices'][y]['Description'], 0, True, my_project.current_scenario)
...
for y in range (0, len(origin_destination_dict["Scalar_Matrices"])):
    my_project.create_matrix('SCALAR', origin_destination_dict['Scalar_Matrices'][y]['Name'], 
                             origin_destination_dict['Scalar_Matrices'][y]['Description'], 0, True, my_project.current_scenario)
...
my_project.matrix_calculator(result = 'mohhlds', expression = 'mfhhemp', aggregation_destinations = '+', 
                             constraint_by_zone_origins = '*', constraint_by_zone_destinations = '102-105')
...
refactor_dict = {'moltprof' : 'moltpro * 0.280', 'momtprof' : 'momtpro * 0.547', 'mohtprof' : 'mohtpro*1.125', 
                 'mdltattf' : 'mdltatt*0.749', 'mdmtattf' : 'mdmtatt * 0.500', 'mdhtattf' : 'mdhtatt * 1.375'}
for key, value in refactor_dict.iteritems():
    my_project.matrix_calculator(result = key, expression = value)

print 'done with truck generator script'


#Import dstricts, add truck operating costs
...
#import bi_dir skims to emmebank
...
my_project.matrix_calculator(result = 'mfhvyod', expression = 'mfhvyod + (mfreebee + mfreebei + mfreebie)/264')

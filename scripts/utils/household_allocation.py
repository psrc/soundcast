...
parcel_df = pd.read_csv(
    r"R:\SoundCast\Inputs\2014\landuse\parcels_urbansim.txt", sep=" "
)

# data about where housholds are going and how many. Each dictionary in the list is seperate move or addition.
allocation_list = [
    {"taz_id": 8, "parcel_ids": [1077182], "number_of_households": 200},
...
for hh_dict in allocation_list:
    ...
    freq = get_sample_frequencies(
        hh_dict["number_of_households"], hh_df, "cross_class", [hh_dict["taz_id"]]
    )
    ...
    if move_households:
        hh_df.loc[hh_df.index.isin(sample2.index), ["hhparcel", "hhtaz"]] = sample2[
            ["hhparcel", "hhtaz"]
        ]
        ...
        assert len(hh_df[hh_df.hhparcel == parcel]) > int(
            parcel_df[parcel_df.parcelid == parcel].hh_p
        )
    else:
...
parcel_df["hh_p"] = 0
parcel_df = parcel_df.merge(
    hh_count, how="left", left_on="parcelid", right_on="hhparcel"
)

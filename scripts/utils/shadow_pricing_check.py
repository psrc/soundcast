...
def get_percent_rmse(urbansim_file, daysim_file, guide_file):
    urbansim_data = pd.io.parsers.read_table(
        urbansim_file, sep=" "
    )  # Read in UrbanSim data
    jobs_by_taz = (
        urbansim_data[["taz_p", "emptot_p"]].groupby("taz_p").sum()
    )  # Get number of jobs by TAZ
    daysim_data = h5toDF.convert_single(
        daysim_file, guide_file, "Daysim Outputs", "Person"
    )  # Read in person file from DaySim data
    workers_by_taz = (
        daysim_data["Person"][["pwtaz", "psexpfac"]]
        .query("pwtaz > 0")
        .groupby("pwtaz")
        .sum()
    )  # Get number of workers by TAZ
    workers_jobs_by_taz = pd.merge(
        jobs_by_taz, workers_by_taz, left_index=True, right_index=True
    )  # Merge them
    ...
    workers_jobs_by_taz["Difference"] = (
        workers_jobs_by_taz["DaySim"] - workers_jobs_by_taz["UrbanSim"]
    )
...
def convergence_check(
    rmse_list, convergence_criterion, iteration
):  # Function not presently in use
    if iteration == 1:
        convergence = False
...
def main():
    ...
    current_percent_rmse = get_percent_rmse(
        parcel_decay_file, h5_results_file, guidefile
    )

...
def get_guide(guide_file):
    ...
    for i in guide.sheet_names():
        if guide.sheet_names()[j][len(guide.sheet_names()[j]) - 1] == " ":
            fileguides.update(
                {
                    guide.sheet_names()[j][0 : len(guide.sheet_names()[j]) - 1].encode(
                        "ascii", "replace"
                    ): guide.sheet_by_name(guide.sheet_names()[j])
                }
            )
            j = j + 1
        else:
        ...
        for var in range((len((guide[file].row(0))) + 1) / 2):
            ...
            for cell_value in range(1, len(guide[file].col(2 * var))):
                if guide[file].cell(cell_value, 2 * var).value == "":
                    ...
                    vardict.update(
                        {
                            int(guide[file].cell(cell_value, 2 * var).value): guide[
                                file
                            ]
                            .cell(cell_value, 2 * var + 1)
                            .value
                        }
                    )
            if 0 in vardict:
                ...
                vardict.update(
                    {0: "N\A"}
                )  # If 0 is not a possible entry for the variable and 0 is an actual entry, this converts the entry to 'Error'
            vardict.update(
                {-1: "N\A"}
            )  # Some entries for categorical variables are -1, so this converts those to 'Error'
            catdict.update({vnames[2 * var].value: vardict})
    print(
        "Guide converted to dictionary in "
        + str(round(time.time() - time_start, 1))
        + " seconds"
    )
    return catdict
...
def convert(filename, guidefile, name):
    ...
    if guidefile[L - 4 : L] == "json":
        ...
        for f in input:  # loop through the files
            ...
            for v in input[f]:
                if (
                    type(input[f][v][0]) is np.void
                ):  # If entries are in the form '(number,)', this loop converts them to number
            ...
            print(
                f
                + " File import/recode complete in "
                + str(round(time.time() - fs, 1))
                + " seconds"
            )
        print(
            "---"
            + name
            + " import/recode complete in "
            + str(round(time.time() - ts, 1))
            + " seconds---"
        )
        return output

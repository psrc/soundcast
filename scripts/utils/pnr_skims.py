...
def process(my_project, sc_tod):
    ...
    matrix_list = ['pnr_demand','auto_pnr_distance','auto_pnr_time','pnr_aivt','pnr_boardings',
               'pnr_aux_time','pnr_initial_wait','pnr_total_wait']

    for type in ['access','egress']:
        for matrix_name in matrix_list:
            # Delete matrix if it already exists
            try:
                my_project.delete_matrix(my_project.bank.matrix(matrix_name+'_'+type).id)
                print('-----')
                print(matrix_name)
            except:
                pass
    ...
    my_project.create_extra_attribute('LINK', '@volax_w_all', description='aux w pnr all', overwrite=True)
    my_project.create_extra_attribute('LINK', '@volax_x_all', description='aux x pnr all', overwrite=True)
    my_project.create_extra_attribute('LINK', '@volax_w_pnr', description='aux x pnr', overwrite=True)
    my_project.create_extra_attribute('LINK', '@volax_x_pnr', description='aux x pnr', overwrite=True)
    my_project.create_extra_attribute('LINK', '@volax_y_pnr', description='aux x pnr', overwrite=True)
    my_project.create_extra_attribute('LINK', '@volax_z_pnr', description='aux (x pnr', overwrite=True)
    ...
    for asim_name, matrix_name in matrix_dict.items():
        try:
            ...
            h5file["Skims"].create_dataset(asim_name+'__'+asim_tod, data=matrix_value.astype('uint16'),compression='gzip')
        except:
            print(matrix_name)
    ...
    pnr_total_wait_access = emmeMatrix_to_numpyMatrix('pnr_total_wait_access', my_project.bank, 'uint16', 100)
    pnr_initial_wait_access = emmeMatrix_to_numpyMatrix('pnr_initial_wait_access', my_project.bank, 'uint16', 100)
    pnr_transfer_wait_access = pnr_total_wait_access-pnr_initial_wait_access
    h5file["Skims"].create_dataset('DRV_LOC_WLK_XWAIT__'+asim_tod, 
                                   data=pnr_transfer_wait_access.astype('uint16'),compression='gzip')

    pnr_total_wait_egress= emmeMatrix_to_numpyMatrix('pnr_total_wait_egress', my_project.bank, 'uint16', 100)
    pnr_initial_wait_egress = emmeMatrix_to_numpyMatrix('pnr_initial_wait_egress', my_project.bank, 'uint16', 100)
    pnr_transfer_wait_egress = pnr_total_wait_egress-pnr_initial_wait_egress
    h5file["Skims"].create_dataset('WLK_LOC_DRV_XWAIT__'+asim_tod, 
                                   data=pnr_transfer_wait_access.astype('uint16'),compression='gzip')

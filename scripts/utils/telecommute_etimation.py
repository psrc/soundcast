...
survey_path = Path('R:/e2projects_two/SoundCast/Inputs\dev/base_year/2018/survey/RSG_version')
...
parcels = pd.read_csv(r'R:\e2projects_two\SoundCast\Inputs\dev\landuse\2018\v3_RTP\parcels_urbansim.txt', sep= ' ')
...
person_day['person_id'] = person_day['hhno'].astype(str) + '_' + person_day['pno'].astype(str)
...
person_day = person_day.merge(person, how='left', left_on='person_id', right_on='person_id')
...
commuters['wkathome'] = np.where(commuters['wkathome']>=hours_telecommuting_threshold, 1, 0)
...
commuters['income_50plus'] = np.where(commuters['hhincome']>=50000, 1, 0) # not used in estimation
...
commuters['children_plus_nwa'] = np.where((commuters['children']>0) & (commuters['hhoad']>0), 1, 0)
...
commuters['frac_industrial_inc_0_50'] = commuters['fraction_industrial_jobs'] * commuters['income_0_50']
commuters['frac_industrial_inc_50plus'] = commuters['fraction_industrial_jobs'] * commuters['income_50plus']

commuters['fraction_office_jobs']= commuters['empofc_p']/commuters['emptot_p']
commuters['frac_office_inc_0_50'] = commuters['fraction_office_jobs'] * commuters['income_0_50']
commuters['frac_office_inc_50plus'] = commuters['fraction_office_jobs'] * commuters['income_50plus']

commuters['fraction_gov_jobs']= commuters['empgov_p']/commuters['emptot_p']
commuters['frac_gov_inc_0_50'] = commuters['fraction_gov_jobs'] * commuters['income_0_50']
commuters['frac_gov_inc_50plus'] = commuters['fraction_gov_jobs'] * commuters['income_50plus']

commuters['fraction_retail_jobs']= (commuters['empret_p'] + commuters['empfoo_p']) /commuters['emptot_p']
commuters['frac_retail_inc_0_50'] = commuters['fraction_retail_jobs']  * commuters['income_0_50']
commuters['frac_retail_inc_50plus'] = commuters['fraction_retail_jobs'] * commuters['income_50plus']
...
log_reg = smf.logit("wkathome ~  pwautime + part_time_worker + missing_income + income_0_50 + income_150plus + no_vehicles + fraction_medical_jobs + fraction_other_jobs + frac_industrial_inc_0_50 + frac_industrial_inc_50plus + frac_office_inc_0_50 + frac_office_inc_50plus + frac_gov_inc_0_50 + frac_gov_inc_50plus + frac_retail_inc_0_50 + frac_retail_inc_50plus", data=commuters).fit()

...
class Person(pa.DataFrameModel):
    ...
    pemploy: int = pa.Field(isin=e.EmploymentType)  # employment status
    pstudent: int = pa.Field(isin=e.StudentType)  # student status
...
class Landuse(pa.DataFrameModel):
    ...
    FOOEMPN: Food and beverage employment
    ...
    def check_tot_employment(cls, land_use: pd.DataFrame):
        tot_emp = land_use[
            ["RETEMPN", "FPSEMPN", "HEREMPN", "OTHEMPN", "AGREMPN", "MWTEMPN", "FOOEMPN"]
        ].sum(axis=1)
